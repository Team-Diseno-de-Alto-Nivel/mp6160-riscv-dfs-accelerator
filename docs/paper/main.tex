...
The proposed DFS accelerator was developed following a design methodology of the hardware in which the functional validation was prioritized. The project starts from the description of the high-level software intead of implementing the architecture directly in RTL and then it is refined it into a hardware-oriented model. This development strategy reduces debugging effort from the beginning of the project.


\subsection{Overall Development Flow}

The development process starts with a software reference model of the DFS algorithm and progressively refines it into a hardware implementation for FPGA deployment. This incremental approach allows the design to be verified at each stage before continuing with the next one. Figure~\ref{fig:development_flow} illustrates the main stages of this process.
...
SystemC was adopted as the main modeling language during the early stages of the accelerator development. The goal was to validate the behavior of the DFS architecture before moving to an RTL implementation. Using SystemC/TLM made it possible to model the communication between the hardware modules and verify the functionality of the accelerator while keeping the design at a higher level of abstraction.
