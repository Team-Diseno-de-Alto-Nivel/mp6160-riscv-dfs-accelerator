...
This kind of development follows a refinement methodology, in which the accelerator moves forward from a software reference into a complete FPGA-based hardware system. Figure~\ref{fig:development_flow} illustrates the stages of this process.
...
SystemC was adopted as the modeling language during the early stages. The objetive of this was to verify the behavior of the DFS architecture before moving into an RTL implementation. Using SystemC/TLM made it possible to model the communication between the hardware modules and verify the functionality of the accelerator while keeping the design at a higher level of abstraction.
...
The communication scheme follows a structure similar to an AXI-Lite peripheral, the transition from the SystemC model to the FPGA implementation requires minor architectural changes. This approach reduces the hardware integration process and allows the validated model to be reused during the synthesis stages.
