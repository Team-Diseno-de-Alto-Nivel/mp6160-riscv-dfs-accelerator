...
This overhead comes from invoking the accelerator itself, not from the
DFS computation.
...
magnitude as the hardware measurement. We leave closing this gap as
future work, which requires an additive fixed-overhead term in the cost
model, not just retuning \texttt{cycles\_per\_op}/\texttt{clock\_period\_ns}.
