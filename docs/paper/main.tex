...
& Splits the graph so the work is shared across several FPGAs and performance does not stall on a single chip. Its load balancing and chip-to-chip communication hold up well.
& Synchronizing multiple FPGAs unleashes a development nightmare. Leading to leave this out if you are working with embedded systems or low-cost boards.
...
The proposed DFS accelerator was developed following a hardware design methodology in which functional validation prior to hardware synthesis was prioritized. Rather than implementing the architecture directly in RTL, the project starts from the description of the high-level software and gradually refines it into a hardware-oriented model. This development strategy reduces debugging effort, allows architectural exploration at an early stage, and minimizes propagating design errors risks into the final hardware implementation.


\subsection{Overall Development Flow}

The development process follows a refinement methodology, in which the accelerator moves forward from a software reference implementation into a complete FPGA-based hardware system. Figure~\ref{fig:development_flow} illustrates the main stages of this process.
...
SystemC was chosen as the primary modeling language for this project because it provides a suitable balance between a high-level description of the system and an close representation of hardware behavior. At the transaction level, it allows the behavior of the accelerator, its communication interfaces, and its internal hardware modules to be verified without the complexity of traditional RTL development.
...
To improve scalability, maintainability, and support future hardware optimizations, the accelerator was implemented using a modular architecture instead of a single complex block. Each module is responsible for a specific task and communicates with the rest of the system through the defined interfaces. This organization provides a structure similar to a real hardware accelerator and reduces future modifications or extensions.

\subsection{Accelerator Architecture}

The proposed accelerator architecture is composed of independent hardware modules which are controlled by a centralized DFS controller. Figure~\ref{fig:accelerator_architecture} presents the organization of the main components.
...
\node[block] (controller)
{DFS\\Controller};

\node[block, below left=1.3cm and 4cm of controller] (stack)
{Stack\\Manager};

\node[block, below left=1.3cm and 2cm of controller] (neighbor)
{Neighbor\\Generator};

\node[block, below=1.8cm of controller] (visited)
{Visited\\Memory};

\node[block, below right=1.3cm and 2cm of controller] (grid)
{Grid\\Memory};

\node[block, below right=1.3cm and 4cm of controller] (result)
{Result\\Interface};


\foreach \x in {stack,neighbor,visited,grid,result}
{
\draw[arrow] (controller) -- (\x);
}
...
The communication between software and hardware accelerator is modeled through a memory-mapped register interface using Transaction-Level Modeling (TLM). From the software perspective, the accelerator behaves as a conventional memory-mapped peripheral, while internally the SystemC model captures the interactions among the hardware modules.
Since this communication scheme follows a structure similar to an AXI-Lite peripheral, the transition from the SystemC model to the FPGA implementation requires only minor architectural changes. This approach reduces the hardware integration process and allows the validated model to be reused during the synthesis and deployment stages.


\subsection{Hardware Generation Strategy}

AMD Vitis High-Level Synthesis (HLS) was selected as the hardware generation approach instead of manually developing the accelerator using the traditional RTL design. This decision reduces development complexity and allows the hardware implementation to be derived from a higher-level behavioral description.
After the SystemC model has been functionally validated and its timing characteristics have been analyzed, the design can be further refined and synthesized into RTL using Vitis HLS. This enables a smoother transition from architectural modeling to hardware implementation while preserving the validated functionality.
...
\pend{Resource usage, Fmax, and OP/s from the Vitis HLS/Vivado synthesis on
the Kria KV260 are in progress and will be added once available.}
